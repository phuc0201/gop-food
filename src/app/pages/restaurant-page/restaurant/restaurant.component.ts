...
export class RestaurantComponent implements OnInit, AfterViewInit {
  ...
  selectResCate(index: number): void {
    ...
    const restaurantInfor = this.el.nativeElement.querySelector(`.restaurant-infor`);
    const header = document.getElementById('header');

    if (contentElement && header) {
      const contentOffsetTop = contentElement.offsetTop;
      const tabsContainerBottom = this.tabsContainer.nativeElement.getBoundingClientRect().bottom;
      if (restaurantInfor.getBoundingClientRect().bottom - header.offsetHeight > 0) {
        const scrollPosition = (contentOffsetTop - tabsContainerBottom) + (restaurantInfor.getBoundingClientRect().bottom - header.offsetHeight);
